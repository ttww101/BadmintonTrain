...
class SimPlayVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let team1 = teamRecordModel(name: simPlayModel.home, score: 0, quarter1: 0, quarter2: 0, quarter3: 0, quarter4: 0, quarter5: 0, Drop: 0, Drive: 0, Smash: 0, Out: 0)
        let team2 = teamRecordModel(name: simPlayModel.away, score: 0, quarter1: 0, quarter2: 0, quarter3: 0, quarter4: 0, quarter5: 0, Drop: 0, Drive: 0, Smash: 0, Out: 0)
    ...
    func recordGame() {
        ...
        proGRecord.team1.Drop += manager.homeDrop
        proGRecord.team1.Drive += manager.homeDrive
        proGRecord.team1.Smash += manager.homeSmash
        proGRecord.team1.Out += manager.homeOut
        proGRecord.team1.score = game1
        
        proGRecord.team2.Drop += manager.awayDrop
        proGRecord.team2.Drive += manager.awayDrive
    ...
    // MARK: - UI Event
    @IBAction func addDropPressed(_ sender: UIButton) {
        switch sender.tag {
        case 0:
            manager.addPoint(.home, .drop)
            score1 += 1
        case 1:
            manager.addPoint(.away, .drop)
            score2 += 1
        default:
            return
        }
    ...
    @IBAction func addDrivePressed(_ sender: UIButton) {
        switch sender.tag {
        case 0:
            manager.addPoint(.home, .drive)
            score1 += 1
        case 1:
            manager.addPoint(.away, .drive)
            score2 += 1
        default:
            return
        }
    ...
    @IBAction func addSmashPressed(_ sender: UIButton) {
        switch sender.tag {
        case 0:
            manager.addPoint(.home, .smash)
            score1 += 1
        case 1:
            manager.addPoint(.away, .smash)
            score2 += 1
        default:
            return
        }
    ...
    @IBAction func minusDropPressed(_ sender: UIButton) {
        switch sender.tag {
        case 0:
            manager.minusPoint(.home, .drop) {[weak self] (success) in
                if success { self?.score1 -= 1 }
            }
        case 1:
            manager.minusPoint(.away, .drop) {[weak self] (success) in
    ...
    @IBAction func minusDrivePressed(_ sender: UIButton) {
        switch sender.tag {
        case 0:
            manager.minusPoint(.home, .drive) {[weak self] (success) in
                if success { self?.score1 -= 1 }
            }
        case 1:
            manager.minusPoint(.away, .drive) {[weak self] (success) in
                if success { self?.score2 -= 1 }
            }
        default:
            return
    ...
    @IBAction func minusSmashPressed(_ sender: UIButton) {
        switch sender.tag {
        case 0:
            manager.minusPoint(.home, .smash) {[weak self] (success) in
                if success { self?.score1 -= 1 }
            }
        case 1:
            manager.minusPoint(.away, .smash) {[weak self] (success) in
                if success { self?.score2 -= 1 }
            }
        default:
            return
